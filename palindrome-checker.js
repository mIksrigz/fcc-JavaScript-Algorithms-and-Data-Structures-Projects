...
function palindrome(str) {
    const preparedString = str.match(/[^\W_+]/g).join("").toLowerCase();
    let startPointer = 0;
    let endPointer = preparedString.length - 1;
    while(startPointer < endPointer){
        if(preparedString.charAt(startPointer) !== preparedString.charAt(endPointer)) return `${preparedString} is not a plaindrome.`;
        startPointer++;
        endPointer--;
    }
